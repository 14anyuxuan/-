/********************************************************************
* 文件名:   ls1d6_std_uart.h
* 版本：	  V1.0
* 作者:    李鑫
* 日期:    2021.09.30
* 说明:    本文件包含串口uart的底层驱动库函数声明及宏定义
* 版权:    龙芯中科（太原）技术有限公司
********************************************************************/
#ifndef _UART_H_
#define _UART_H_
...
#define UART0_IR_MODE   0  // 1 红外模式，0串口模式

#define IR_CarrierWave_MODE   0
/**
 * @说明  GPIO 初始化定义
 */
typedef enum
...
#define	UART_FIFO_TRIGGER_4			         ((uint8_t)0x40)	/* trigger at 4 chars */
#define	UART_FIFO_TRIGGER_8			         ((uint8_t)0x80)	/* trigger at 8 chars */
#define	UART_FIFO_TRIGGER_14			     ((uint8_t)0xC0)	/* trigger at 14 char */
#define IS_UART_FIFO_TRIGGER(TRIGGER) (((TRIGGER) == UART_FIFO_TRIGGER_1) || ((TRIGGER) == UART_FIFO_TRIGGER_4) || \
        					((TRIGGER) == UART_FIFO_TRIGGER_8)||((TRIGGER) == UART_FIFO_TRIGGER_14))
...
#define UART_IR_PULSE_DIV_SHIFT                   (uint32_t)0
#define UART_IR_PULSE_DIV_MASK                    (uint32_t)0xffff
#define UART_IR_PULSE_CLKSEL_SHIFT                (uint32_t)16
#define UART_IR_PULSE_CLKSEL_MASK                 (uint32_t)0x10000
#define UART_IR_PULSE_ENABLE_SHIFT                (uint32_t)17
#define UART_IR_PULSE_ENABLE_MASK                 (uint32_t)0x20000


#define UART_IR_DISABLE   0
#define UART_IR_ENABLE   1
